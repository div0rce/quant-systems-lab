(* Tests:
   - the exported valid fixture and the two reuse fixtures must pass all invariants
     (a rejected/canceled id may be legally reused by a later accept);
   - the two violation fixtures must fail (proving the checker still catches real errors). *)
...
let check path expect_ok =
  ...
  if ok <> expect_ok then (
    ...
    List.iter (fun (n, o, d) -> Printf.eprintf "  [%b] %s %s\n" o n d) checks;
    exit 1)

let () =
  check "fixtures/valid.txt" true;
  check "fixtures/valid_rejected_reuse.txt" true;
  check "fixtures/valid_canceled_reuse.txt" true;
