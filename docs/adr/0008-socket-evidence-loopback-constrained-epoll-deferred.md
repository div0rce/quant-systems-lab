# ADR 0008: Socket Evidence Is Loopback-Constrained and epoll Is Deferred
...
2. An `epoll`-based event-driven gateway is a natural next step, but `epoll` is a Linux-specific
   API that cannot be compiled or tested on the macOS development host.
...
- M30 profiles and hardens the **existing** one-connection-at-a-time gateway rather than rewriting
  it. The `epoll` multi-client architecture is deferred to **M34**, and multi-client
  socket-pressure testing to **M35**, so no untested Linux-only code is committed now. `io_uring`
  is discussed only.
...
later milestones (M34/M35) and acknowledged evidence debts, rather than implicit gaps or
untested code in the tree.
